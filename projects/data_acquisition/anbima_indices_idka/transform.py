...
from projects.data_acquisition.anbima_indices_idka.constants import (
    ITF,
    INTERVALO,
    FAMCOML,
    ORIGINAL_COLUMNS,
...
from projects.data_acquisition.anbima_indices_idka.data_contract import anbima_indices_idka_schema
import pandas as pd
from typing import List


class TransformAnbimaIDkA(TransformData):
    ...
    def transform_data(
        ...
        for blob_name in blob_path:
            ...
            df_pre_output = self.remove_useless_columns(dataframe=df_raw)
        ...
        output_data = self.create_output_dict(execution_date=exec_date, dataframe=df_transformed, identifier=ITF, schema=anbima_indices_idka_schema)
    ...
    def remove_useless_columns(dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        df.drop(columns=['tx_compra', 'tx_venda'], inplace=True, errors='ignore')
        return df
